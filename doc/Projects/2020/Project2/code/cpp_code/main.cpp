...
int main(int argc, char const* argv[]){
  ...
  string algorithm = string(argv[5]);

  double h = (rho_N-rho_0)/N;

  mat V = zeros<mat>(N-1,N-1); // Diagonal matrix with potentials for each rho
  for(int i=0; i<N-1; i++){
    V(i,i) = potential(rho_0 + (i+1)*h);// make matrix then fill, or init diag mat from vec
  }
  ...
  cout << "\n With Lapack:"<<endl;
  cout << real(Reigval.subvec(0,3))<<endl;
  ...
  if(algorithm == "Jacobi"){
    ...
    cout << jacobi_solver.Get_eigvals(0,3) << endl;
  ...
  if(algorithm == "Lanczos"){
    ...
    cout << "Eigenvalues:" << endl;
    cout << lanczos_solver.Get_eigvals(0,3) << endl;
    cout << "Time used: " << algotime << " s" << endl;
  }
