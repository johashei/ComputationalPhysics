...
///*
  SIRS solver(a0,b,c);
  solver.Set_state(S,I,R);
  solver.Vital_dynamics(e,d,dI);
  solver.Seasonal_variation(A,omega);
  solver.Vaccination(f,pf0);
  solver.Solve_MC(Tfinal,1,"tests.tsv");

  /*
  int N = S+I+R;
  double t = 0;

  double minTs[] = {4/((A+a0)*N*N), 1/(b*N), 1/(c*N), 1/(e*N), 1/(d*N), 1/(dI*N), 1/f0};
  double dt = *min_element(minTs,minTs+7);
  cout <<  dt << endl;
  int MCs = int(Tfinal/dt);
