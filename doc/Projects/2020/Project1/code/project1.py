...
try:
    sys.argv[3] == "-d"
    os.system("rm ./"+path+"/*") # delete datafiles from previous runs
except:
    pass
...
Times = np.zeros(len(exponents)) # store timer values in ms


# Call the C++ solver if necessary and read the datafile:
for i in range(len(exponents)):
    ...
    u = np.zeros(n+2) # store numerical solution
